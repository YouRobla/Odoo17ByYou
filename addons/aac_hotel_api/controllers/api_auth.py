...
class ApiKeyController(http.Controller):
    ...
    def generate_api_key(self, **kw):
        ...
        if not name:
            return self._prepare_response(
                {"success": False, "error": 'El campo "name" es requerido'}, status=400
            )
        ...
        try:
            ...
            plaintext_key = apikey_sudo._generate(scope=scope, name=name)
            ...
            return self._prepare_response(
                {"success": False, "error": f"Error al generar API key: {str(e)}"},
                status=500,
            )
    ...
    def get_my_api_keys(self, **_kw):
        ...
        api_keys = (
            ...
            .sudo()
            .search([("user_id", "=", request.env.user.id)], order="create_date desc")
        )
            ...
            keys_data.append(
                {
                    ...
                    "name": key.name,
                    "created_at": (
                        key.create_date.isoformat() if key.create_date else None
                    ),
        ...
        return self._prepare_response(
            {"success": True, "count": len(keys_data), "data": keys_data}
        )
    ...
    def revoke_api_key(self, key_id, **_kw):
        ...
        if not api_key_record.exists():
            return self._prepare_response(
                {"success": False, "error": f"API key con ID {key_id} no encontrada"},
                status=404,
            )
        ...
        return self._prepare_response(
            {"success": True, "message": f'API key "{key_name}" revocada exitosamente'}
        )
    ...
    def validate_api_key_public(self, **kw):
        ...
        try:
            ...
            return self._prepare_response(
                {"success": False, "valid": False, "error": "Error al validar API key"},
                status=500,
            )
    ...
    def test_api_key(self, **kw):
        ...
        if not api_key:
            return self._prepare_response(
                {"success": False, "error": "Debe proporcionar la API key a probar"},
                status=400,
            )
